...
pub(crate) fn certificat(domain: String) -> CertificateRecord
{
	...
	let check_ssl = CheckSSL::from_domain(domain.as_str());
	match check_ssl
	{
		Ok(certificate) => {
			...
			extensions_server.subject_alternative_names = certificate.server.sans;
			...
			extensions_intermediate.subject_alternative_names = certificate.intermediate.sans;
		}
